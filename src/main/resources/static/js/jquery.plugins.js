...
(function($) {
    ...
    $.JQPlugin.createPlugin({
        ...
        _generateContent: function(h, i) {
            ...
            var t = this;
            function removeHighlight() { (i.inline ? $(this).closest('.' + t._getMarker()) : i.div).find(i.options.renderer.daySelector + ' a').removeClass(i.options.renderer.highlightedClass);
            }
                ...
                for (var y = 0; y < 7; y++) {
                    ...
                    if (b.options.rangeSelect && b.selectedDates.length > 0) {
                        ...
                        for (var i = 0; i < b.selectedDates.length; i++) {
                            selectedTime = b.selectedDates[i].getTime();
                    ...
                    if (b.get('isStart')) {
                        ...
                        if (b.get('maxDate') != null) {
                            endTime = b.get('maxDate').getTime();
                        }
                    } else {
                        if (b.get('minDate') != null) {
                            startTime = b.get('minDate').getTime();
                        }
                        endTime = selectedTime;
                    }

                    if (startTime != null) {
                        if (startTime === s.getTime()) {
                            z = true;
                        }
                    }

                    if (endTime != null) {
                        if (endTime === s.getTime()) {
                            z = true;
                        }
                    }
                    ...
                    x += this._prepare(e.day, b).replace(/\{day\}/g, (B ? '<a href="javascript:void(0)"': '<span') + ' class="dp' + t + ' ' + (A.dateClass || '') + (z && (n || s.getMonth() + 1 === d) ? ' ' + e.selectedClass: '') + (B ? ' ' + e.defaultClass: '') + ((s.getDay() || 7) < 6 ? '': ' ' + e.weekendClass) + (s.getMonth() + 1 === d ? '': ' ' + e.otherMonthClass) + (s.getTime() === r.getTime() && (s.getMonth() + 1) === d ? ' ' + e.todayClass: '') + ((s.getTime() === b.drawDate.getTime() || range) && (s.getMonth() + 1) === d ? ' ' + e.highlightedClass: '') + '"' + (A.title || (b.options.dayStatus && B) ? ' title="' + (A.title || F.formatDate(b.options.dayStatus, s, b.getConfig())) + '"': '') + '>' + (b.options.showOtherMonths || (s.getMonth() + 1) === d ? A.content || s.getDate() : '&#160;') + (B ? '</a>': '</span>'));
                    F.add(s, 1, 'd');
                    t = s.getTime();
                }
    ...
    var F = $.datepick;
    $(function() {
        $(document).on('mousedown.' + E, F._checkExternalClick).on('resize.' + E,
            function() {
                F.hide(F.curInst)
            })
    })
